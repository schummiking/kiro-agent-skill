...
/**
 * Bridge Session Recovery — Preservation Property Tests
 *
 ...
 * 4. ping/pong response format (no controlMode/controlPath in stdio mode)
 * 5. unknown op error
...
function log(msg) {
  console.log(`[session-recovery-pres] ${msg}`);
}
...
async function testPingPong() {
  ...
  if ('controlMode' in response) checks.push(`unexpected controlMode: ${response.controlMode}`);
  if ('controlPath' in response) checks.push(`unexpected controlPath: ${response.controlPath}`);
  if (checks.length === 0) {
    record('ping/pong', true, 'Correct format, no controlMode/controlPath');
  } else {
...
async function testBridgeSignalReceivedFields() {
  ...
  if (!signalEvent) {
    record('bridge_signal_received fields', false, `No event. Events: ${JSON.stringify(events.map(e => e.type))}`);
    return;
  }
  const checks = [];
...
async function main() {
  log('Starting session recovery preservation property tests');
