...
- **One-shot tasks** — run a bounded coding task via `kiro-cli chat --no-interactive`
- **ACP orchestration** — multi-turn programmatic control via `kiro-cli acp` and a lightweight JSON-RPC bridge
...
| Scenario | Transport |
|---|---|
| Multi-turn orchestration | `kiro-cli acp` via `kiro-acp-bridge.js` |
...
- **一次性任务** — 通过 `kiro-cli chat --no-interactive` 执行有边界的编程任务
- **ACP 编排** — 通过 `kiro-cli acp` 和轻量级 JSON-RPC 桥接实现多轮程序化控制
...
| 场景 | 传输方式 |
|---|---|
| 多轮编排 | `kiro-cli acp` 通过 `kiro-acp-bridge.js` |
| 一次性任务 | `kiro-cli chat --no-interactive --trust-all-tools` |
